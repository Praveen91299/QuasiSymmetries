"""Generate and benchmark frozen-core F2/STO-3G Clifford symmetry data for JUL20.

The systems are F--F geometries with the F 1s cores frozen, giving a
CAS(14e, 8o) active-space Hamiltonian, i.e. 16 qubits:

* F2frozen_eqm:  R(F--F) = 1.412 Angstrom
* F2frozen_corr: R(F--F) = 2.000 Angstrom
* F2frozen_diss: R(F--F) = 3.000 Angstrom
...
def f2_geometry(f_f_distance: float):
    return [
        ("F", (0.0, 0.0, -f_f_distance / 2.0)),
        ("F", (0.0, 0.0, f_f_distance / 2.0)),
    ]
...
def active_space_info(molecule):
    core_orbitals = list(range(NCORE))
    active_orbitals = list(range(NCORE, molecule.n_orbitals))
    active_electrons = molecule.n_electrons - 2 * NCORE
    return core_orbitals, active_orbitals, active_electrons


def generate_hamiltonian(system: str, f_f_distance: float, ham_dir: Path, text):
    ...
    print(f"Geometry: F-F, R(F-F) = {f_f_distance:.3f} Angstrom", file=text)

    molecule = MolecularData(
        geometry=f2_geometry(f_f_distance),
        basis=BASIS,
    ...
    core_orbitals, active_orbitals, active_electrons = active_space_info(molecule)

    H = get_fermion_operator(
    ...
    hf_occ = get_hf_occ(active_electrons, len(active_orbitals), as_str=True)
    ...
    print(f"full n_electrons: {molecule.n_electrons}", file=text)
    print(f"full n_orbitals: {molecule.n_orbitals}", file=text)
    print(f"frozen core orbitals: {core_orbitals}", file=text)
    print(f"active orbitals: {active_orbitals}", file=text)
    print(f"active electrons: {active_electrons}", file=text)
...
def load_or_generate_hamiltonian(system: str, f_f_distance: float, ham_dir: Path, text, force: bool):
    pkl_path, mol_path = hamiltonian_paths(ham_dir, system)
    if force or not pkl_path.exists():
        return generate_hamiltonian(system, f_f_distance, ham_dir, text)
...
def generate_bliss_hct_dataset(H, HQ, cisd_gs, fci_gs, molecule, n_qubits: int, text):
    singleton_metric, _beam_score = make_commutator_metric(HQ, cisd_gs, n_qubits)
    _core_orbitals, _active_orbitals, active_electrons = active_space_info(molecule)

    print("\nRunning Pauli BLISS before HCT(n)...", file=text)
    print(f"active n_electrons used for BLISS: {active_electrons}", file=text)
...
def benchmark_system(system: str, distance: float, args, text):
    ...
    print("=" * 80, file=text)
    core_orbitals, active_orbitals, active_electrons = active_space_info(molecule)
    print(f"R(F-F) = {distance:.3f} Angstrom", file=text)
    print(f"n_qubits = {n_qubits}", file=text)
    print(f"active_electrons = {active_electrons}", file=text)
    print(f"active_orbitals = {len(active_orbitals)}", file=text)
    print(f"FCI energy = {fci_e}", file=text)
    print(f"CISD energy = {cisd_e}", file=text)
    ...
    system_result = {
        "geometry": {
            "description": "F-F with frozen F 1s cores",
            "f_f_distance_angstrom": distance,
            "basis": BASIS,
            "multiplicity": MULTIPLICITY,
            "charge": CHARGE,
            "ncore": NCORE,
...
def main():
    ...
    text_file = args.output_dir / "jul20_f2_clifford_benchmarks.txt"
    json_file = args.output_dir / "jul20_f2_clifford_benchmarks.json"
    dmrg_csv = args.output_dir / "jul20_f2_dmrg_bond_dimensions.csv"
    mpo_csv = args.output_dir / "jul20_f2_mpo_bond_dimensions.csv"
    runtime_csv = args.output_dir / "jul20_f2_runtime_memory.csv"
    ...
    all_results = {
        ...
        "systems": {},
        "geometry_notes": {
            "F2frozen_eqm": "R(F-F)=1.412 Angstrom, near experimental equilibrium.",
            "F2frozen_corr": "R(F-F)=2.000 Angstrom, stretched correlated regime.",
            "F2frozen_diss": "R(F-F)=3.000 Angstrom, dissociated/stretched regime.",
        },
        "settings": {
            "beam_width": args.beam_width,
            "heavy_core_fraction": args.heavy_core_fraction,
    ...
    with text_file.open("w", buffering=1) as text:
        print("JUL20 frozen-core F2/STO-3G Clifford/Fiedler benchmarks", file=text)
