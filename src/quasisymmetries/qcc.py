# Qubit Coupled Cluster, code by Rob Lang
...
class IterativeQCC:
    ...
    def init_from_molecule(cls, molecule):
        if isinstance(molecule, MolecularData):
            molecule = tq.Molecule.from_openfermion(molecule)
        
        hamiltonian = molecule.make_hamiltonian()
        ref_circ =  molecule.prepare_reference()
        ref_occs = np.zeros(2 * molecule.n_orbitals)
        np.put(ref_occs, ref_circ.qubits, 1)
        return cls(hamiltonian, ref_occs)
    
    @classmethod
    def init_from_hamiltonian(cls, hamiltonian, ref_occs = None):
        if isinstance(hamiltonian, QubitOperator):
            hamiltonian = tq.QubitHamiltonian.from_openfermion(hamiltonian)
        n_qubits = hamiltonian.n_qubits
        if ref_occs == None:
            ref_occs = np.array([0]*n_qubits)
        return cls(hamiltonian, np.array(ref_occs))
    
    def __init__(self, hamiltonian, ref_occs):
        self.hamiltonian = hamiltonian
        self.n_qubits = hamiltonian.n_qubits
        self.ref_occs =  ref_occs
        def prepare_ref_circ(ref_occs):
            circuit = tq.QCircuit()
            for i, a in enumerate(ref_occs):
                if a == 1:
                    circuit += tq.gates.X(target = i)
            return circuit
        self.ref_circ = prepare_ref_circ(ref_occs)
        np.put(self.ref_occs, self.ref_circ.qubits, 1)
        self.binary_repr = [
            pauli.binary(n_qubits=self.n_qubits) for pauli in self.hamiltonian.paulistrings
        ]
    ...
    def optimize(self, method="COBYLA", x0=None, maxiter=None):
        """
        ...
        noise = tq.circuit.noise.DepolarizingError(p=0.1, level=2)
        qcc_energy = tq.ExpectationValue(H=self.hamiltonian, U=self.ref_circ + self.qcc_circ, noise = noise)
    ...
    def do_qcc(self, n_gen=1, method="COBYLA", grad_threshold=1e-6, x0=None, maxiter=None):
        self.select(n_gen=n_gen, grad_threshold=grad_threshold)
        self.get_qcc_unitary()
        self.optimize(method=method, x0=x0, maxiter=maxiter)

    def do_iteration(
        self, n_gen=1, method="COBYLA", grad_threshold=1e-6, compression_threshold=1e-8, x0=None, maxiter=None
    ):
        self.do_qcc(n_gen=n_gen, method=method, grad_threshold=grad_threshold, x0=x0, maxiter=maxiter)
